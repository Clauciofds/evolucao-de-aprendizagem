...
- Teste automatizados eu entendia como vários processo automaticos gerado pelo desenvolvedor para verificar seu código mesmo antes da impletação dos processos em si. Como isso tentar mitigar erros de lógica e saidas que não condizem com as regras de negócio do projeto.
...
- Gostaria de entender e aplicar testes automatizados em meus projetos de tal forma que eu possa posteriomente assim futuramente entender e ler teste escritos por outros desenvolvedores e ter como melhorá-los quando possível.
...
- Consequi entender com muito mais clareza a aplicabilidade e a relevância dos teste automatizados no mercado profissional. E o desenvovimento de teste usando o VS Code com exercícios praticos em testes unitários de um função.

- Consegui entender com muito mais clareza a aplicabilidade e a relevância dos testes automatizados no mercado profissional. E o desenvolvimento de teste usando o VS Code com exercícios práticos em testes unitários de uma função. 

 

- Aprendi que um servidor pode ser mais que apenas um hardware e que pode estar rodando em sua máquina compartilhando recursos e hopendando APIs baseado no rest utilizando o express ou Fastify. E ainda: 

    - Entendi a estrutur e a comuniciação realizado através de solicitações via protocolo HTTP  e como configura um servidor Nodes.js a usar a capaciade de enviar e receber comunicações HTTP. 

    - O manuseio de armazenamento e recuperação de dados. 

    - A utilização de bibliotecas especificas para operação com banco de dados.

    - Produtividade no desenvolvimento de aplicações server-side com Nodes.js. 

    - Tive contato e utilizamos a biblioteca Fastify para configurar o acesso a um servidor via URL  e porta utilizando as rotas no Typescript 

    - Utilizando o framework Express entendi como implementar um servidor DDS e também definir rotas utilizando do mesmo. Implementações de método para receber e enviar requisições a um servidor através de alocação de portas para sua API. 

    - Foi passado ferramentais para proteger dados sensíveis utilizando o process.env da biblioteca env-cmd evitando o rastreamento desses dados e também utilizando a função ou pasta .gitignore do versionador de código GitHub para ter mais uma camada de segurança a nossas aplicações. 

    - Entendi o gerenciamento das variáveis de ambiente através do ECMAScript, CommonJS e a biblioteca externa dotenv se utilizando do require e processos nativo do Nodes.js. 

    - Tive contato e exercitei os conceitos de estruturação do código para rodar uma API rest distinguindo o que são controladores, intermediários, utilitários, etc. E é claro a manipulação de dados e informações dentro destas estruturas. 

    - Exercitei os fundamentos do processo de requisições e resposta ao servidor através do protocolo HTTP (Hyper Tex Transfer Protocol) e os métodos GET, POST, PUT, DELETE e sua funções. Além dos códigos de status HTTP. 

    - A diferença entre servidores web e servidores de aplicativos: 

        - Servidor web: Apache, Nginx, etc. 

        - Servidor de aplicações: Node.js, Django, Rails, etc. 

    - Compreendi como o DNS (Domain Name System) é traduzido em um IP. 

    - Sobre segurança na web vimos um pouco de Criptografia e Certificados SSL e Cross-site scripting e injeção SQL. 

    - Entendi o que são cookies e como são usados. 

    - Aprendi a diferença entre o uso de parâmetros (Params) e consulta (Query) para detalhar requisições 
